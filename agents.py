"""
agents.py — Kipaji-AI Hybrid Agent Pipeline
Agent 1 (Bias Guardrail): Gemini 2.0 Flash (Nuanced cultural understanding)
Agent 2 (Underwriter): Groq Llama 3.3 (Sub-second latency for USSD)
"""
...
async def _call_groq(client, prompt: str, system: str = "") -> str:
    def _blocking():
        response = client.chat.completions.create(
            model=MODEL_ID_GROQ, messages=[{"role": "system", "content": system}, {"role": "user", "content": prompt}],
...
async def run_bias_mitigation_guardrail(raw_message: str, merchant_id: str, language: str, gemini_client=None) -> SanitizedInput:
    if gemini_client is None: return _guardrail_fallback(raw_message, language)
    prompt = f"Merchant message (language hint: {language}):\n\"\"\"\n{raw_message}\n\"\"\"\nExtract trade events and apply bias filtering. Return only JSON."
    try:
        raw_response = await _call_gemini(gemini_client, prompt, system=BIAS_GUARDRAIL_SYSTEM)
        clean = raw_response.strip()
        start, end = clean.find('{'), clean.rfind('}')
        if start != -1 and end != -1 and end > start: clean = clean[start:end+1]
        else: raise ValueError("No JSON object found")
        sanitized = SanitizedInput(**json.loads(clean))
        logger.info(f"[{merchant_id}] Guardrail (Gemini): {len(sanitized.detected_trade_events)} events, confidence={sanitized.overall_confidence:.2f}")
        return sanitized
...
def _guardrail_fallback(raw_message: str, language: str) -> SanitizedInput:
    return SanitizedInput(cleaned_text=raw_message, detected_trade_events=[TradeEvent(event_type="unknown", amount_ksh=None, confidence=0.1, raw_utterance=raw_message[:200], bias_flags=[])], language=language, low_confidence_count=1, overall_confidence=0.1, bias_proxy_removed=[])
...
async def run_kipaji_underwriter_core(sanitized: SanitizedInput, history: List[Dict[str, Any]], profile: Dict[str, Any], velocity_metrics: Dict[str, float], merchant_id: str, groq_client=None) -> CreditDecision:
    ...
    try:
        ...
        logger.info(f"[{merchant_id}] Decision (Groq): {decision.credit_tier} | KSH {decision.approved_local}")
        return decision
    except Exception as e:
...
def _low_confidence_decision(sanitized: SanitizedInput, merchant_id: str) -> CreditDecision:
    msg = "Samahani, hatukuelewa vizuri. Tafadhali tuambie: uliuza nini leo na bei ngapi?" if sanitized.language == "sw" else "Sorry, we couldn't understand your message clearly."
