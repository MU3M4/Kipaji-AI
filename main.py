...
Fixes applied vs v2.1:
  - Agent functions now exist and are imported (agents.py)
  - Gemini calls run in asyncio.to_thread (never blocks event loop)
  - get_merchant_data fully implemented for Firestore path
  - save_trade_interaction stores extracted trade amount, NOT credit output
  - avg_daily always divides by 7-day window, not transaction count
  - Language detection augmented with lexical pre-check for Sheng/Dholuo
  - USSD session state via Redis (storage.py)
  - Typed TradeEventSchema enforced across agent boundary
  - Confidence gate: routes to clarification prompt below 0.65
  - CORS locked to explicit origins
  - WebSocket audit is fire-and-forget (never on credit critical path)
  - /health endpoint for uptime monitoring
  - /api/v1/ussd endpoint for telecom gateway integration
"""
...
class USSDRequest(BaseModel):
    ...
    serviceCode: str
    phoneNumber: str
    text: str                   # accumulates across menus e.g. "1*2*500"
...
def calculate_velocity_metrics(history: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    High-frequency traders were previously penalised by using len(sales) as denominator.
    """
    sales = [
        t for t in history
    ...
    recent = sales[-7:]   # at most the last 7 entries (one per day idealised)
...
async def inbound_telecom_gateway(
    ...
    2. Merchant data retrieval (Firestore or in-memory)
    ...
    # Store the EXTRACTED TRADE AMOUNT, not the approved credit amount
...
async def ussd_gateway(request: Request, background_tasks: BackgroundTasks):
    ...
    Each USSD session is a multi-turn conversation collected before AI processing.

    USSD flow:
    Step 0 → Welcome + ask for trade description
    Step 1 → Collect trade message → run AI pipeline → return decision
    Step END → Display result, clear session
    """
    ...
    if step == 1:
        ...
        if decision.confidence_gate_triggered:
            ...
            await save_ussd_session(session_id, session, redis_client=redis_client)
            # Truncate to 182 chars (USSD screen limit)
